...
- Status: `[x]`
- Checkpoint: `O — Rebuild the test suite against the post-rewrite codebase — complete`
- Subtask: `Test suite rebuilt and green: 388 passed, 0 failed, 0 collection errors. Ran as eight bounded packets — O1 inventory + a resolver-UX audit; the global lookup_place bare->station resolver fix with a resolved-as echo (d777622b; contract doc 888e9ca7), the only production change of the whole checkpoint; then test alignment: resolver parity rewritten onto TradeORM (de39bb0e) with the retired-TradeDB/cache modules removed (86b71719) and the station-lookup test aligned (8104fd1a); commandenv/CLI two-tier backend (f395332c); GUI argv/snapshot/backend (191b3dd9); plugin/import/buildcache lifecycle (2657a6c7); direct/run commands (f25affc1). Every alignment verified behaviour against the production code rather than rubber-stamping output; no defect surfaced beyond the O2 resolver regression. Two non-blocking tidies recorded, not actioned: the buildcache deprecation NOTE still names ensure_fresh_db though the code calls the local _rebuild_database, and the suite carries a shared pytest fixture-import flake8 pattern (F401/F811 on isolated_trade_env). Per-packet reports under docs/CHECKPOINT_O_* (untracked). O is complete. Next: P (document the project).`
- Owner: `Tromador`
- Started: `2026-06-24`
- Goal: `Restore a green, meaningful test suite against the post-rewrite codebase — remove or rewrite tests bound to retired modules, realign coverage to current behaviour, and record a green baseline as the gate for later work. See the Checkpoint O detail for the task breakdown and commits.`
...
- [x] O — Rebuild the test suite against the post-rewrite codebase
...
### Tasks
- [x] O1. Inventory the suite: passing, failing, and obsolete-by-design
  - Status note: `Inventoried the collectable suite (366 tests + 3 collection-error modules) and classified every failure: obsolete retired-architecture, fixture drift, candidate current-code defect, or unclear. The one "real defect" candidate — bare-name place resolution — was traced next and proved a genuine resolver regression (fixed under O2). A follow-up resolver-UX audit mapped name resolution across every command.`
  - Evidence: `docs/CHECKPOINT_O_PACKET_1_INVENTORY.md; docs/CHECKPOINT_O_RESOLVER_UX_AUDIT.md`
- [x] O2. Remove or rewrite tests bound to archived modules (tradedb/tradecalc/preload)
  - Status note: `Removed the pure-TradeDB/cache test modules (test_cache, test_peek, test_tradedb_lookup_db, test_tradedb_routing_db) and the now-unused isolated_tdb helper (86b71719); rewrote the legacy TradeDB resolver-parity module onto TradeORM as a focused contract guardrail (de39bb0e); aligned the station-lookup test to the station-only contract (8104fd1a). Retired four-tier-backend / preload expectations were dropped from the commandenv/CLI and GUI suites in the same wave.`
  - Evidence: `commits 86b71719, de39bb0e, 8104fd1a`
- [x] O3. Add or repair coverage for the planner and the post-L command surface
  - Status note: `Restored the global lookup_place bare->station contract and added the resolved-as echo plus tests — the one production change of the whole checkpoint (d777622b; contract doc 888e9ca7). Realigned coverage to current behaviour across the commandenv/CLI two-tier backend (f395332c), the GUI argv/snapshot/backend paths (191b3dd9), the plugin/import/buildcache lifecycle (2657a6c7), and the direct/run commands (f25affc1). Each alignment verified behaviour against the production code rather than rubber-stamping output; no defect surfaced beyond the O2 resolver regression.`
  - Evidence: `commits d777622b, 888e9ca7, f395332c, 191b3dd9, 2657a6c7, f25affc1`
- [x] O4. Confirm a full green run and record it as the new baseline
  - Status note: `Full suite green: 388 passed, 0 failed, 0 collection errors (pytest -q on 2026-06-24). Recorded as the new baseline. Across the whole checkpoint only one production change landed (the O2 resolver fix); everything else was test alignment — the rewrite's behaviour was already correct and the tests had lagged it.`
  - Evidence: `pytest -q -> 388 passed; pytest --collect-only -q -> 388 collected, 0 errors`
...
- Checkpoint O is complete (2026-06-24). The rebuild ran as eight bounded packets —
  inventory + a resolver-UX audit, the resolver contract fix, then five
  test-alignment buckets (resolver, commandenv/CLI, GUI, plugin/import, direct/run).
  Per-packet reports are kept under `docs/CHECKPOINT_O_*` (untracked working notes).
- Two non-blocking tidies were surfaced and recorded, not actioned (out of each
  packet's scope): the `buildcache` deprecation NOTE still names `ensure_fresh_db`
  though the code calls the local `_rebuild_database`; and the test suite carries a
  suite-wide pytest fixture-import flake8 pattern (F401/F811 on the
  `isolated_trade_env` import) shared by several modules.
...
- [x] Milestone: `Checkpoint O complete — test suite rebuilt against the post-rewrite codebase; full suite green`
  - Date completed: `2026-06-24`
  - Commit: `f25affc1` (O closeout); rebuild commits 86b71719, 8104fd1a, de39bb0e, f395332c, 191b3dd9, 2657a6c7, f25affc1; resolver fix d777622b (+ contract doc 888e9ca7)
  - Notes: `pytest -q -> 388 passed, 0 failed, 0 collection errors. Eight bounded packets: inventory + resolver-UX audit, the global lookup_place bare->station resolver fix (the only production change), then test alignment across resolver parity, commandenv/CLI two-tier backend, GUI, plugin/import/buildcache, and direct/run. Retired TradeDB/cache/tier/preload tests removed or rewritten onto TradeORM/planner; the rest were stale expectations realigned to verified current behaviour. New baseline for later work.`
