...
- Subtask: `L5, L1, L2 and L3 complete (range-option contract; olddata SQL-first; nav rebuilt as a basic A->B plotter on the planner's plan_jump_path, --refuel-jumps dropped to leave advanced fuel/neutron routing to Spansh; L3 rare cutover verified clean — buy --rare on the canonical Item.rare_station_id predicate, no live refs to the retired rares command, no code change). Remaining in L: L4 (trade trade -> trade direct), L6 (re-evaluate remaining full TradeDB.load callers). Then M (GUI compatibility + session reuse), N (closeout), O (test-suite rebuild). The full test suite is broken by the planner rewrite — tracked as O; command work is smoke-validated, not gated on pytest. GUI deferred to M.`
- Owner: `Tromador`
- Started: `2026-06-22`
