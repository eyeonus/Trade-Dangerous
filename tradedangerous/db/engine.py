...
def _get(cfg: Dict[str, Any], section: str, key: str, default: Any | None = None) -> Any:
...
def _get_int(cfg: Dict[str, Any], section: str, key: str, default: int | None = None) -> int | None:
    val = _get(cfg, section, key, default)
    try:
        return int(val)
    except (TypeError, ValueError):
        return default

def _get_bool(cfg: Dict[str, Any], section: str, key: str, default: bool | None = None) -> bool | None:
    val = _get(cfg, section, key, default)
...
def make_engine_from_config(cfg: Dict[str, Any]) -> Engine:
    """Construct a SQLAlchemy Engine for MariaDB or SQLite.

    Side-effect free beyond ensured path creation for SQLite/data dirs.
    """
    ...
    # Ensure base dirs exist (plugins rely on them even under MariaDB)
    ...
    # attach a redacted URL for optional logging (attribute on Engine)
    try:
...
def get_session_factory(engine: Engine):
    """Return a configured sessionmaker (expire_on_commit=False)."""
    return sessionmaker(bind=engine, expire_on_commit=False, autoflush=True)

def healthcheck(engine: Engine, retries: int = 0) -> bool:
    """Run a simple 'SELECT 1'. Set retries>0 to retry on OperationalError."""
    attempt = 0
    delay = 0.25
    while True:
        try:
