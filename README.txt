...
around one of the most powerful trade run optimizers (TRO) available.
...
The price data in TradeDangerous is either manually entered (by you) or
crowd sourced (e.g. http://www.davek.com.au/td).
...
example "update -F -G -A" can be written as "update -FGA".
...
For additional help on a specific command, such as 'update' use
  ...
  trade.py local …
    Lists systems (and optionally, stations) in the vicinity
    of a given system
...
IMPORT sub-command and plugins:
 ...
 Import:
  ...
  Provides mechanisms for loading data, epsecially price data, into the local
  database, using either "import" or "merge" modes.

  Both modes operate on a station-by-station basis: you can import data for
  a single station or every station.
  ...
  - Items are removed from stations when there is an explicit "0 0" entry
    or there is no entry for the item at the station in the import file.

  Merge Mode (--merge):
  - Items are only imported if they are newer than the local data,
  - Items are removed from stations only when there is an explicit "0 0"
    entry that is newer than the local data.
...
NAV sub-command:
  ...
  given a ship, it uses the maximum dry range of the ship. Use --full if you
